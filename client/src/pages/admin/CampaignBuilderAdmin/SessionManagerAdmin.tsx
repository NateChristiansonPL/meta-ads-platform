...
export default function SessionManager({ state, onLoad }: Props) {
  ...
  const { data: sessions = [], isLoading: loadingSessions } = trpc.adminSessions.list.useQuery(
    undefined,
    { enabled: isAuthenticated && open }
  );

  const saveMutation = trpc.adminSessions.save.useMutation({
    onSuccess: (data) => {
  ...
  const deleteMutation = trpc.adminSessions.delete.useMutation({
    onSuccess: () => {
